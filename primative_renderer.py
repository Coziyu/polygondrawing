...
def render_point(point, color, window):
...
def render_bezier(p1, c1, c2, p2, color, window):
    ...
    for i in range(len(line_segs) - 1):
        pygame.draw.line(window, color, line_segs[i], line_segs[i + 1])

def render_connection(p1, p2, p3, color, window):
    if len(p1) == 2 and len(p2) == 2:
            # Draw a line
            render_line(p1, p2, color, window)
        ...
        render_bezier(p1, control_point_1, control_point_2, p3, color, window)
    elif len(p1) == 4:
        pass
    else:
